...
# 步骤2：创建带音频功能的MainActivity类（使用内部类而不是匿名内部类）
...
public class MainActivity extends Activity {
    ...
    private void startAudioTest() {
        ...
        // 创建录音线程
        recordingThread = new Thread(new AudioProcessingRunnable());
...
# 步骤4：生成dex文件（使用通配符处理所有class文件）
"${BUILD_TOOLS}/d8" "${PROJECT_DIR}/temp_build/com/example/carkaraoke/*.class" --lib "${PLATFORM}/android.jar" --output "${PROJECT_DIR}/temp_build"
...
if [ -f "${PROJECT_DIR}/temp_build/classes.dex" ]; then
    ...
    echo "警告：classes.dex文件不存在，跳过添加步骤"
fi
