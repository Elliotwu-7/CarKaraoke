...
# 步骤5：使用aapt2编译和链接AndroidManifest.xml
"${BUILD_TOOLS}/aapt2" compile --dir "${PROJECT_DIR}/temp_build" -o "${PROJECT_DIR}/temp_build/compiled_res.flat" 2>/dev/null
"${BUILD_TOOLS}/aapt2" link "${PROJECT_DIR}/temp_build/compiled_res.flat" -o "${PROJECT_DIR}/temp_build/resources.apk" -I "${PLATFORM}/android.jar" --manifest "${PROJECT_DIR}/temp_build/AndroidManifest.xml" 2>/dev/null

# 步骤6：准备APK文件结构
mkdir -p "${PROJECT_DIR}/temp_build/apk"

# 如果resources.apk存在，提取它，否则直接创建必要的目录
if [ -f "${PROJECT_DIR}/temp_build/resources.apk" ]; then
    ...
    # 直接复制AndroidManifest.xml到apk目录
    cp "${PROJECT_DIR}/temp_build/AndroidManifest.xml" "${PROJECT_DIR}/temp_build/apk/"
fi
...
if [ -f "${PROJECT_DIR}/${APK_NAME}" ]; then
    ...
    echo "APK结构："
    unzip -l "${PROJECT_DIR}/${APK_NAME}" | head -20
else
    echo "构建失败！"
fi
